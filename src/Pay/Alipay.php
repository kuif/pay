...
class Alipay
{
     ...
     */
    public function __construct($config=NULL, $is_sandbox = false){
        $config && self::$config = array_merge(self::$config, $config);
        isset(self::$config['sign_type']) && self::$signType = self::$config['sign_type'];
        self::$gateway = $is_sandbox ? self::$sandurl : self::$apiurl; //如果是沙箱测试
	...
	public static function face($order){
        $order['scene'] = 'bar_code'; // 支付场景。bar_code(默认)：当面付条码支付场景； security_code：当面付刷脸支付场景，对应的auth_code为fp开头的刷脸标识串；
    ...
    public function getToken()
    {
        ...
        if (isset($_GET['auth_code'])){
            ...
            $url = 'https://openauth.' . (self::$is_sandbox ? 'alipaydev' : 'alipay') . '.com/oauth2/publicAppAuthorize.htm?' . $bizString;
            Header("Location: $url");
            exit();
        }
    }
    ...
    protected static function buildRequestForm($url, $params) {
        ...
        $sHtml = "正在跳转至支付页面...<form id='alipaysubmit' name='alipaysubmit' action='".$url."?charset=".self::$charset."' method='POST'>";
